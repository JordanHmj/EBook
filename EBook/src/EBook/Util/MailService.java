...
public class MailService {
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail,String userName) throws Exception {
        ...
        message.setFrom(new InternetAddress(sendMail, "肖恩分享小站", "UTF-8"));

        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, userName, "UTF-8"));

        // 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
        message.setSubject("肖恩分享小站账号激活", "UTF-8");

        // 5. Content: 邮件正文（可以使用html标签）（内容有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改发送内容）
        message.setContent("您好，<a href='http://localhost:8080/EBook/HostActivation?sign="+ShaunUtils.StringToHex(receiveMail)+"'>请点击此处激活账号</a>", "text/html;charset=UTF-8");
